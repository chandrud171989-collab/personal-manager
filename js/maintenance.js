(() => {
  "use strict";
  const PM = window.PM;
  const TYPES = ["AC","RO / Water Purifier","Refrigerator","Washing Machine","Geyser","Vehicle Service","Home Cleaning","Pest Control","Electrical/Plumbing","Other"];
  const REM = [30,7,1];
  ...
  async function load() {
    ...
    return (data || []).map(row => {
      ...
      if (match) { cost = Number(match[1]); notes = notes.replace(/(?:^|\n)Service cost:\s*₹[\d.]+/i,"").trim(); }
      return {...row, cost, notes};
    });
  }

  async function saveExpense(item) {
    if (!item.cost || !Number.isFinite(Number(item.cost)) || Number(item.cost) <= 0) return;
    try {
      const serviceDate = item.last_service_date || new Date().toISOString().slice(0,10);
      const payload = {
        user_id: PM.user.id,
        maintenance_id: item.id,
        item_name: item.item_name || item.category || "Other item",
      ...
      <div class="section">
        ...
        ${rows.length ? rows.map(m => {
          ...
          return `<div class="card status-${PM.statusClass(days)}" data-id="${PM.escape(m.id)}">
            <div class="card-main"><div class="card-title">${PM.escape(m.item_name || m.category || "Other item")}</div>
            <div class="card-sub">${PM.escape(m.category || "Maintenance")} · next service ${PM.escape(PM.dateText(m.next_service_date))}</div></div>
  ...
  function openDetail(m) {
    PM.modal(`
      <div class="modal-title">${PM.escape(m.item_name || m.category || "Maintenance item")}</div>
      <div class="detail-row"><span>Type</span><strong>${PM.escape(m.category || "Other")}</strong></div>
  ...
  function openForm(old = null) {
    ...
    PM.modal(`
      <div class="modal-title">${edit ? "Edit maintenance" : "Add maintenance"}</div>
      <form id="maintForm">
        <div class="field"><label>Item</label><select id="type" name="type">${TYPES.map(t=>`<option ${old?.category===t ? "selected":""}>${PM.escape(t)}</option>`).join("")}</select></div>
        <div class="field" id="customNameWrap" style="display:${old?.category==="Other" ? "block":"none"}"><label>Custom name</label><input id="customName" name="itemName" value="${PM.escape(old?.category==="Other" ? (old?.item_name||"") : "")}" placeholder="e.g. Water heater"></div>
        ...
        <div class="field"><label>Service due date</label><input type="date" name="nextServiceDate" value="${PM.escape(old?.next_service_date || "")}" min="${today}" required><small>Today or a future date only.</small></div>
        ...
        <div class="field"><label>Remind me before service</label>
          <div class="chip-row" id="maintRem">${REM.map(n=>`<button type="button" class="chip-toggle ${reminder===n?"on":""}" data-val="${n}">${n} day${n>1?"s":""}</button>`).join("")}
          <button type="button" class="chip-toggle ${reminder && !REM.includes(reminder)?"on":""}" id="customRemBtn">Custom</button></div>
          <div id="maintCustomWrap" style="display:${reminder && !REM.includes(reminder)?"block":"none"};margin-top:12px"><label style="text-transform:none">Custom reminder (days)</label><input id="maintCustom" type="number" min="1" max="365" value="${reminder && !REM.includes(reminder)?reminder:""}"></div>
        </div>
    ...
    type.onchange = () => customNameWrap.style.display = type.value === "Other" ? "block" : "none";
    ...
    document.getElementById("maintForm").onsubmit = async e => {
      e.preventDefault();
      const btn = document.getElementById("saveMaint");
      const status = document.getElementById("maintStatus");
      btn.disabled = true; btn.textContent = "Saving...";
      try {
        const fd = new FormData(e.target);
        const category = String(fd.get("type") || "Other");
        ...
        const itemName = category === "Other" ? (String(fd.get("itemName")||"").trim() || "Other item") : category;
        const userNotes = String(fd.get("notes")||"").trim();
        const fullNotes = [userNotes, costText ? `Service cost: ₹${Number(costText)}` : ""].filter(Boolean).join("\n") || null;

        status.textContent = "Saving...";
        const record = {
          id, user_id: PM.user.id, item_name: itemName, category,
          last_service_date: last, next_service_date: next,
        ...
        await saveExpense({...record, cost: costText ? Number(costText) : null, notes: userNotes});
        PM.closeModal();
        render(await load());
      } catch (e) {
        console.error(e);
  ...
  function renderExpense(rows, from, to) {
    ...
    btn.dataset.rows = JSON.stringify({rows,from,to});
  }
  ...
  async function renderExpenseSummary() {
    ...
    try { document.getElementById("expenseResults").innerHTML='<div class="empty">Loading expenses...</div>'; renderExpense(await fetchExpenses(from,to),from,to); }
  ...
  function downloadXLS() {
    ...
    if(!raw) return;
    const {rows,from,to}=JSON.parse(raw);
    const byItem={}; rows.forEach(r=>{const k=r.item_name||"Other item";byItem[k]=(byItem[k]||0)+Number(r.amount||0)});
    const summary=Object.entries(byItem).map(([k,v])=>`<tr><td>${PM.escape(k)}</td><td>${Number(v).toFixed(2)}</td></tr>`).join("");
    const details=rows.map(r=>`<tr><td>${PM.escape(r.service_date||"")}</td><td>${PM.escape(r.item_name||"Other item")}</td><td>${Number(r.amount||0).toFixed(2)}</td><td>${PM.escape(r.notes||"")}</td></tr>`).join("");
    const html=`<!doctype html><html><head><meta charset="utf-8"><style>body{font-family:Arial}table{border-collapse:collapse}th,td{border:1px solid #999;padding:7px 10px}</style></head><body><h1>Personal Manager - Maintenance Expenses</h1><p>Period: ${PM.escape(from||"All")} to ${PM.escape(to||"All")}</p><h2>Expense Summary</h2><table><tr><th>Item</th><th>Total</th></tr>${summary}</table><h2>Expense Details</h2><table><tr><th>Date</th><th>Item</th><th>Amount</th><th>Notes</th></tr>${details}</table></body></html>`;
    const blob=new Blob([html],{type:"application/vnd.ms-excel"});
    const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`maintenance-expenses-${from||"all"}-to-${to||"all"}.xls`; document.body.appendChild(a); a.click(); a.remove(); setTimeout(()=>URL.revokeObjectURL(url),1000);
  }

  async function start() {
    try { await PM.initPage("maintenance"); document.getElementById("fab").onclick=()=>openForm(); render(await load()); }
