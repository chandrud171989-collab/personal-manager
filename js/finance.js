(() => {
  ...
  function render() {
    view.innerHTML = `
      ...
      <div class="section">
        <div class="section-head"><span class="section-title">Home Expenses</span><button class="btn primary" id="addExpense">+ Add Expense</button></div>
        ...
        <div class="modal-actions">
          <button class="btn primary" id="viewHome">View Expenses</button>
          <button class="btn" id="downloadHome" disabled>Download Excel (.xls)</button>
        </div>
      ...
      <div class="section">
        ...
        <button class="btn primary" id="calcEmi">Calculate EMI</button>
  ...
  function openExpenseForm(old=null) {
    ...
    PM.modal(`
      ...
      <form id="expenseForm">
        ...
        <div class="field"><label>Category</label><input name="category" value="${PM.escape(old?.category||"Other")}" placeholder="e.g. Food, Electricity, Travel"></div>
    ...
    document.getElementById("expenseForm").onsubmit=async e=>{
      ...
      try{
        ...
        const rec={
          id:old?.id||PM.uuid(),user_id:PM.user.id,
          name:String(fd.get("name")||"").trim(),
          category:String(fd.get("category")||"Other").trim()||"Other",
          amount:Number(fd.get("amount")),
          expense_date:String(fd.get("expenseDate")),
          notes:String(fd.get("notes")||"").trim()||null,
          updated_at:new Date().toISOString()
        };
        if(!rec.name||!rec.amount||!rec.expense_date)throw new Error("Please complete the required fields.");
  ...
  async function viewHome() {
    try {
      ...
      let q=PM.client.from("finance_expenses").select("*").eq("user_id",PM.user.id).order("expense_date",{ascending:false});
      if(from)q=q.gte("expense_date",from); if(to)q=q.lte("expense_date",to);
      const {data,error}=await q; if(error)throw error;
      const home=data||[];
      let mq=PM.client.from("maintenance_expenses").select("item_name,amount,service_date,notes").eq("user_id",PM.user.id);
      if(from)mq=mq.gte("service_date",from); if(to)mq=mq.lte("service_date",to);
      const {data:maint,error:me}=await mq; if(me)throw me;
      ...
      const byCat={}; home.forEach(r=>{const k=r.category||"Other";byCat[k]=(byCat[k]||0)+Number(r.amount||0);});
      ...
      document.getElementById("homeResults").innerHTML=`
        ...
        <div class="section"><div class="section-title">Home Expenses</div><div class="finance-grid">
        ...
        <div class="section"><div class="section-title">Expense by Item / Category</div><table class="summary-table"><tr><th>Item</th><th>Total</th></tr>${Object.entries(byCat).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<tr><td>${PM.escape(k)}</td><td>₹${PM.money(v)}</td></tr>`).join("")||"<tr><td colspan='2'>No expenses</td></tr>"}</table></div>
        <div class="section"><div class="section-title">Expense Details</div><table class="summary-table"><tr><th>Date</th><th>Item</th><th>Amount</th></tr>
          ${home.map(r=>`<tr><td>${PM.escape(PM.dateText(r.expense_date))}</td><td>${PM.escape(r.name)}</td><td>₹${PM.money(r.amount)}</td></tr>`).join("")}
          ${maintenance.map(r=>`<tr><td>${PM.escape(PM.dateText(r.service_date))}</td><td>${PM.escape(r.item_name||"Maintenance")}</td><td>₹${PM.money(r.amount)}</td></tr>`).join("")}
        </table></div>`;
      const btn=document.getElementById("downloadHome"); btn.disabled=!(home.length||maintenance.length);
      btn.dataset.rows=JSON.stringify({home,maintenance,from,to});
    } catch(e){document.getElementById("homeResults").innerHTML=`<div class="error">${PM.escape(e.message||e)}</div>`;}
  }
  ...
  function downloadHome() {
    ...
    const blob=new Blob([html],{type:"application/vnd.ms-excel"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`finance-expenses-${from||"all"}-to-${to||"all"}.xls`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);
  }
  ...
  function calculateEMI() {
    ...
    document.getElementById("emiResult").innerHTML=`<div class="section"><div class="finance-result-row finance-highlight"><span>Monthly EMI</span><strong>₹${PM.money(emi)}</strong></div><div class="finance-result-row"><span>Total Payment</span><strong>₹${PM.money(total)}</strong></div><div class="finance-result-row"><span>Total Interest</span><strong>₹${PM.money(interest)}</strong></div><div class="finance-result-row"><span>Number of Payments</span><strong>${n}</strong></div></div>`;
  }

  (async()=>{try{await PM.initPage("finance");render();}catch(e){view.innerHTML=`<div class="error">${PM.escape(e.message||String(e))}</div>`;}})();
})();